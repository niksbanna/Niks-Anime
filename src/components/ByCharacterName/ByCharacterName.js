...
export default function ByCharacterName() {
    ...
    const handleOnClick = (event) => {
        ...
        if (chrName.current.length > 0) {
            // waifu.it API supports filtering by author (character)
            fetch(`${WAIFU_API.quote}?author=${encodeURIComponent(chrName.current)}`)
    ...
    return (
        <div className='min-vh-100'>
            <section className="mw7 center " id='bycharactername'>
                <hr />
